...
static int twl_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alm)
{
	...
	if (alm->enabled)
	{
		printk("alex twl_rtc_set_alarm alarm_irq_enable year = %d mon =% d mday = %d hour = %d min =%d sec = %d !!\n", alm->time.tm_year,alm->time.tm_mon,alm->time.tm_mday,alm->time.tm_hour,alm->time.tm_min,alm->time.tm_sec );
		ret = twl_rtc_alarm_irq_enable(dev, 1);
	}

	/**alex set current time***/
	/***************************/

	ret = twl_rtc_read_time(dev, &tm);
	if (ret < 0) {
		printk("alex rtc error\n");
		return ret;
	}



	printk("alex twl_rtc_set_alarm alarm_irq_enable current time year = %d mon =% d mday = %d hour = %d min =%d sec = %d !!\n", tm.tm_year,tm.tm_mon,tm.tm_mday,tm.tm_hour,tm.tm_min,tm.tm_sec );
	/**************************/
out:
	return ret;
}
...
static int __devinit twl_rtc_probe(struct platform_device *pdev)
{
	...
	rtc = rtc_device_register(pdev->name, &pdev->dev, &twl_rtc_ops, THIS_MODULE);
	if (IS_ERR(rtc)) {
		ret = PTR_ERR(rtc);
	...
	{
		struct rtc_time tm;
		printk("%s: Power is lost, set default time\n", __func__);
		tm.tm_sec = 0;
		tm.tm_min = 0;
		tm.tm_hour = 0;
		tm.tm_mday = 1;
...
static int twl_rtc_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (ret < 0) {
		printk("alex twl_rtc_suspend twl_rtc_read_u8 1 error !!\n");
	}

	
		//alex
	twl_i2c_read_u8(TWL4030_MODULE_INT,&rd_reg, REG_GPIO_IMR1A);
	if (ret < 0) {
		printk("alex twl_rtc_suspend twl_rtc_read_u8 2 error !!\n");
	}
