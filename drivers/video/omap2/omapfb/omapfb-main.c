...
extern	void *boot_fbuff;//copy of boot frame buffer
...
static void fill_fb_from_buf(struct fb_info *fbi)
{
	u8 *addr = fbi->screen_base;
	u8 *addrs = boot_fbuff;
	u32 i;
	u16 dst;
	...
	if(0 == addrs || 0 == addr)
	{
		printk("fill_fb_from_buf: error pointer %x - %x\n", addrs, addr);
		return;
	}
	printk("fill_fb_from_buf: %dx%d\n", fbi->var.xres, fbi->var.yres);
	for(i = 0; i < fbi->var.xres * fbi->var.yres; i++)
	{
		dst = ((addrs[0] >> 3) & 0x1F) | (((addrs[1] >> 2) & 0x3F) << 5) | (((addrs[2] >> 3) & 0x1F) << 11);
		memcpy(addr, &dst, 2);
		addr += 2;
		addrs += 3;
	}
	kfree(boot_fbuff);//allocated in a300_init
}
#endif
...
static int omapfb_parse_def_modes(struct omapfb2_device *fbdev)
{
	...
	while (!r && (this_opt = strsep(&options, ",")) != NULL) {
		...
		*p = 0;
		...
		display = NULL;
		for (i = 0; i < fbdev->num_displays; ++i) {
			if (strcmp(fbdev->displays[i]->name, display_str) == 0) {
