 ...
 * Use GPIO_0 of TPS65950 as card detect
 * TODO: return back the common GPIO handling
 *
...
#define TWL_GPIO_CD_IRQ				(1<<0) //twl gpio_1
#endif
/*
...
static int omap_hsmmc_get_wp(struct device *dev, int slot)
{
	...
	if (mmc->version != MMC_CTRL_VERSION_2)
	{
		int ret = gpio_get_value_cansleep(mmc->slots[0].gpio_wp);

		printk("%s line:%d, gpio=%d\n", __FUNCTION__, __LINE__, ret);
...
static int omap_hsmmc_suspend_cdirq(struct device *dev, int slot)
{
	...
	if (mmc->version != MMC_CTRL_VERSION_2)
	{
		...
		if(0 < twl_i2c_read_u8(TWL4030_MODULE_GPIO, &val, REG_GPIO_IMR1A))
			twl_i2c_write_u8(TWL4030_MODULE_GPIO, val | TWL_GPIO_CD_IRQ, REG_GPIO_IMR1A);
#else
...
static irqreturn_t omap_hsmmc_cd_handler(int irq, void *dev_id)
{
	...
	schedule_delayed_work(&host->mmc_carddetect_work,msecs_to_jiffies(500));
