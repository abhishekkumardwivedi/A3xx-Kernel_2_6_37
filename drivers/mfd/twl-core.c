...
int twl_i2c_write(u8 mod_no, u8 *value, u8 reg, unsigned num_bytes)
{
	...
	if (unlikely(mod_no > TWL_MODULE_LAST)) {
		printk("%s: invalid module number %d\n", __func__, mod_no);
		return -EPERM;
	}
	...
	if (unlikely(!inuse)) {
		printk("%s: client %d is not initialized\n", __func__, sid);
		return -EPERM;
	}
	mutex_lock(&twl->xfer_lock);
//	if(TWL4030_MODULE_PM_RECEIVER == mod_no && 3 == reg)
//		printk("%s: mod: %04x, reg: %04x, len: %d, val: %x\n", __func__, mod_no, reg, num_bytes, *value);
	/*
...
//		printk("%s: complete %d\n", __func__, ret);
	...
	if (ret != 1) {
		printk("%s: i2c_write failed to transfer all messages, %d\n", __func__, ret);
		if (ret < 0)
			return ret;
		else
			return -EIO;
...
int twl_i2c_read(u8 mod_no, u8 *value, u8 reg, unsigned num_bytes)
{
	...
	mutex_lock(&twl->xfer_lock);
	//printk("%s: mod: %04x, reg: %04x, len: %d\n", __func__, mod_no, reg, num_bytes);
	/* [MSG1] fill the register address data */
	msg = &twl->xfer_msg[0];
	msg->addr = twl->address;
	msg->len = 1;
...
//	u8 val;
	...
	printk("%s: *** 2 seconds before wait on  %lld ***\n", __func__, ktime_to_ms(ktime_get()));

    err = twl4030_wd_enable(2);
	if(err)
	{
		pr_err("%s: failure to configure WD\n", DRIVER_NAME);
	}
    printk("%s: *** Disable peripheral 3V %lld ***\n", __func__, ktime_to_ms(ktime_get()));

    suspend_device_irqs();
    __gpio_set_value(27, 0);
