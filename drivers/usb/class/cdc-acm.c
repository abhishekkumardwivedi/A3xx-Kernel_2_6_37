...
static int acm_port_activate(struct tty_port *port, struct tty_struct *tty)
{
	...
	mutex_lock(&acm->mutex);
	if ((acm->disconnected) || (acm->ehcidown == true))
		goto disconnected;
...
static void acm_tty_close(struct tty_struct *tty, struct file *filp)
{
	...
	if (acm->ehcidown == true)//ignore close as ehci will tare down the port
	{
		dev_dbg(&acm->control->dev, "%s %d exit without doing anything\n", __func__, acm->minor);
		return;
	}
...
#ifdef CONFIG_USB_TLT //add CONFIG_USB_TLT=y
