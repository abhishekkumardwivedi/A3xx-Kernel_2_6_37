...
#define TS_INTERRUPT(x) 		gpio_to_irq(x)
...
//#ifdef SHUTDOWN_PORT
	if (gpio_get_value(ts->power_gpio))
	{
...
static int goodix_ts_power(struct goodix_ts_data * ts, int on)
{
	...
	switch(on) 
	{
		case 0:
			...
			msleep(5);
			if(gpio_get_value(ts->power_gpio))	//has been suspend
				ret = 0;
			break;
		case 1:
			gpio_set_value(ts->power_gpio, 0);
			msleep(5);
			if(gpio_get_value(ts->power_gpio))	//has been suspend
				ret = -1;
			else 
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct goodix_i2c_platform_data *pl_data;
	int ret = 0;
//	int retry=0;
	int count=0;
	...
	pl_data = client->dev.platform_data;
	if ( NULL == pl_data ) {
	...
	ts->power_gpio 	= pl_data->power_gpio;
	ts->irq_int 	= pl_data->irq_int;

	//Check I2C function
//#ifdef SHUTDOWN_PORT	
	ret = gpio_request(ts->power_gpio, "TS_SHUTDOWN");	//Request IO
	if (ret < 0) 
	{
		printk(KERN_ALERT "Failed to request GPIO:%d, ERRNO:%d\n",(int)ts->power_gpio,ret);
		goto err_gpio_request;
	}	
	gpio_direction_output(ts->power_gpio, 0);	//Touchscreen is waiting to wakeup
	ret = gpio_get_value(ts->power_gpio);
	if (ret)
	{
...
//#ifdef SHUTDOWN_PORT	
	ts->use_shutdown = 1;
	gpio_set_value(ts->power_gpio, 1);		//suspend
//#endif	
...
//#if defined(INT_PORT)	
	client->irq=TS_INTERRUPT( ts->irq_int );
	if (client->irq)
	{
		ret = gpio_request(ts->irq_int, "TS_INTERRUPT");	//Request IO
		if (ret < 0) 
		{
		...
		ret  = request_irq(TS_INTERRUPT( ts->irq_int ), goodix_ts_irq_handler ,  IRQ_TYPE_EDGE_RISING,
			client->name, ts);
		if (ret != 0) {
			dev_err(&client->dev,"Can't allocate touchscreen's interrupt!ERRNO:%d\n", ret);
			gpio_direction_input(ts->irq_int);
			gpio_free( ts->irq_int );
			goto err_int_request_failed;
		}
		else 
		{	
			disable_irq(TS_INTERRUPT( ts->irq_int ));
			ts->use_irq = 1;
			dev_dbg(&client->dev,"Reques EIRQ %d succesd on GPIO:%d\n",TS_INTERRUPT( ts->irq_int ), ts->irq_int);
		}
	}
//#endif
...
//#endif	
	for(count=0; count<3; count++)
	{
		...
		{
			if(ts->use_irq)
				enable_irq(TS_INTERRUPT( ts->irq_int ));
			break;
		}
	}
	if(ret != 0) {
...
err_init_godix_ts:
	if(ts->use_irq)
	{
		free_irq(TS_INTERRUPT( ts->irq_int ),ts);
//#if defined(INT_PORT)		
		gpio_free(ts->irq_int);
//#endif
	}
