 ...
 *  Copyright (C) 2013 Ran Meyerstein Micronet.
 *
...
#define CONFIG_GPIO_PCA9575_GENERIC_IRQ
...
static int pca9575_gpio_to_irq(struct gpio_chip *gc, unsigned off)
{
	struct pca9575_chip *chip;
...
static void pca9575_setup_gpio(struct pca9575_chip *chip, int gpios)
{
	...
	gc->can_sleep = 0;//1;// gpiolib.c doesn't like me sleeping
...
static void pca9575_irq_work(struct pca9575_chip *chip)
{
	uint16_t input, mask, rising, falling;
	int i;

	pca9575_read_reg(chip, PCA9575_IN, &input);
...
static irqreturn_t pca9575_irq_handler(int irq, void *devid)
{
	struct pca9575_chip *chip = devid;
	struct irq_desc *desc = irq_desc + irq;

	desc->chip->mask(chip->irq);
	desc->chip->ack(chip->irq);
	pca9575_irq_work(chip);
...
static void pca9575_irq_bus_lock(unsigned int irq)
{
	struct pca9575_chip *chip = get_irq_chip_data(irq);

	mutex_lock(&chip->irq_lock);
}

static void pca9575_irq_bus_sync_unlock(unsigned int irq)
{
	struct pca9575_chip *chip = get_irq_chip_data(irq);
	uint16_t new_irqs;
	uint16_t level;
	...
	new_irqs = chip->irq_falling_edge | chip->irq_rising_edge;
	new_irqs &= ~chip->reg_direction;

	while (new_irqs) {
		level = __ffs(new_irqs);
		pca9575_gpio_direction_input(&chip->gpio_chip, level);
		new_irqs &= ~(1 << level);
	}
...
static int pca9575_init_irq(struct pca9575_chip *chip)
{
	...
	int ret, irq, irq_start = chip->irq_start;

	chip->irq = gpio_to_irq(chip->client->irq);
	chip->irq_start = irq_start = chip->irq_base;//gpio_to_irq(chip->gpio_start);
	...
	printk("chip->irq[%d] irq_start[%d] chip->gpio_chip.ngpio[%d] NR_IRQS[%d]\n",chip->irq, irq_start, chip->gpio_chip.ngpio, NR_IRQS);
	...
	ic->bus_lock		= pca9575_irq_bus_lock,
	ic->bus_sync_unlock	= pca9575_irq_bus_sync_unlock,
	...
	ret = request_threaded_irq(chip->irq,
							   ...
							   NULL, chip);
	if (ret) {
		dev_err(NULL, "failed to request irq %d\n",
				chip->irq);
		return -EINVAL;
	}

	chip->gpio_chip.to_irq = pca9575_gpio_to_irq;
				   ...
				   const struct i2c_device_id *id)
{
	...
	chip->last_input = val; //insure first time is caught
	if (ret)
		goto out_failed;
	...
	if(client->irq > 0){
		...
		if (ret)
		{
			printk("pca9575_read_reg: failed\n");
			goto out_failed;
		}
	...
	/* set platform specific interrupt mask */
	ret = pca9575_write_reg(chip, PCA9575_MSK, pdata->imask);
	if (ret)
		goto out_failed;
