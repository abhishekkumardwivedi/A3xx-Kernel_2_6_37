...
int ubi_attach_mtd_dev(struct mtd_info *mtd, int ubi_num, int vid_hdr_offset)
{
	...
	if (err)
	{
		ubi_err("cannot initialize I/O %s(%d)", ubi->bgt_name, err);
		goto out_free;
	}
	...
	if (!ubi->peb_buf1)
	{
		ubi_err("cannot allocate PEB's buffer %s(%d, %d)", ubi->bgt_name, ubi->peb_size, err);
		goto out_free;
	}
	...
	if (!ubi->peb_buf2)
	{
		ubi_err("cannot allocate PEB's buffer 2 %s(%d, %d)", ubi->bgt_name, ubi->peb_size, err);
		goto out_free;
	}
	...
	if (ubi->autoresize_vol_id != -1) {
		...
		if (err)
		{
			ubi_err("cannot auto-resize %s(%d)", ubi->bgt_name, err);
			goto out_detach;
		}
	}
	...
	if (err)
	{
		ubi_err("cannot initialize user interface %s(%d)", ubi->bgt_name, err);
		goto out_detach;
	}
...
static int __init ubi_init(void)
{
	...
	for (i = 0; i < mtd_devs; i++) {
		...
		ubi_msg("open mtd device %d: %s", i, p->name);
		mtd = open_mtd_device(p->name);
		if (IS_ERR(mtd)) {
			err = PTR_ERR(mtd);
			ubi_warn("cannot open mtd device %d: %s", i, p->name);
			goto out_detach;
		}
