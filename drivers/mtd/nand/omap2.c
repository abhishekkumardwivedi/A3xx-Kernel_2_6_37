...
static int __devinit omap_nand_probe(struct platform_device *pdev)
{
	...
	printk("omap_nand_probe: %s, %d\n", pdev->name, pdata->ecc_opt);
	...
	if(pdata->ecc_opt == MICRON_ECC_ONDIE)
	{
	...
	if (info->nand.ecc.mode != NAND_ECC_SOFT) {
		...
		if (pdata->ecc_opt == OMAP_ECC_HAMMING_CODE_HW_ROMCODE) {
			...
			omap_oobinfo.oobfree->length = info->mtd.oobsize - offset - omap_oobinfo.eccbytes;
		} else {
