...
{
	register struct nand_chip *chip = mtd->priv;
	int cnt, status; /* MICRON: local vars used */
	...
	case NAND_CMD_READ0:
		...
		if(!((status = chip->read_byte(mtd)) & NAND_STATUS_READY))
		{
			/*TODO: do something on fail */
			printk(KERN_CRIT "CITICAL ERROR :: NAND not ready: %x\n", status);
		}
		else if(status & NAND_STATUS_FAIL) {
			/*TODO: do something on fail */
			printk(KERN_CRIT "CITICAL ERROR :: Uncorrectable error in page: %d\n", page_addr);
		}
		else if(status & NAND_STATUS_CORRECTED)
		{
			printk(KERN_WARNING "WARNING :: Recoverable error in page %d\n", page_addr);
		}
