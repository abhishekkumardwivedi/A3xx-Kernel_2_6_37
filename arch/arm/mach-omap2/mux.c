...
{
//	printk("omap_mux_write: %X -> %X\n", val, (u32)partition->base + reg);
...
static void omap_mux_init_signals(struct omap_mux_partition *partition,
				  struct omap_board_mux *board_mux)
{
#if !defined (CONFIG_MACH_A317) && !defined(CONFIG_MACH_A300)
			 ...
			 struct omap_ball *package_balls)
{
	...
	mux_partitions_cnt++;
	pr_info("%s: Add partition: #%d: %s, flags: %x\n", __func__, mux_partitions_cnt, partition->name, partition->flags);
