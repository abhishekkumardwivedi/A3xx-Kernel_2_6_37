...
#define OMAP3_SHOW_SOC_FEATURE(feat)		\
	 (omap3_has_ ##feat())?"OK":"NONE"

#define OMAP3_HAS_SOC_FEATURE(feat)		\
	 (omap3_has_ ##feat())
...
static void __init omap3_check_features(void)
{
	...
	OMAP3_CHECK_FEATURE(status, MFREQ1GHZ);
	if (cpu_is_omap3630())
...
{
	seq_printf(m, "L2 Cache: %s\n", OMAP3_SHOW_SOC_FEATURE(l2cache));
	seq_printf(m, "IVA:  %s\n", OMAP3_SHOW_SOC_FEATURE(iva));
	seq_printf(m, "SGX:  %s\n", OMAP3_SHOW_SOC_FEATURE(sgx));
	seq_printf(m, "Neon:  %s\n", OMAP3_SHOW_SOC_FEATURE(neon));
	seq_printf(m, "ISP:  %s\n", OMAP3_SHOW_SOC_FEATURE(isp));

	seq_printf(m, "CLK:  ");
	if (OMAP3_HAS_SOC_FEATURE(192mhz_clk)) seq_printf(m, "192MHZ\n");
	else if (OMAP3_HAS_SOC_FEATURE(720mhz)) seq_printf(m, "720MHZ\n");
	else if (OMAP3_HAS_SOC_FEATURE(mfreq1ghz)) seq_printf(m, "1GHZ\n");
