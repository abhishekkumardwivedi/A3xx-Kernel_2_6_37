...
{
	printk(KERN_INFO "board_nand_init %s\n", (nand_parts && nand_parts->name)?nand_parts->name:"NULL");
	...
	board_nand_data.ecc_opt = MICRON_ECC_ONDIE;
	if(nand_parts->ecclayout)
	{
		printk(KERN_INFO "board_nand_init: layout: ecc bytes %d, first ecc pos %d, free pos %d, free len %d\n",
		nand_parts->ecclayout->eccbytes, nand_parts->ecclayout->eccpos[0],
		nand_parts->ecclayout->oobfree[0].offset, nand_parts->ecclayout->oobfree[0].length);
	}
	else
	{
		printk(KERN_INFO "board_nand_init: layout not defined\n");
	}
#else
	if(cpu_is_omap3630())
