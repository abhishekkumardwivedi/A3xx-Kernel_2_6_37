...
void omap2_init_clk_clkdm(struct clk *clk)
{
	...
	if (clkdm) {
		//printk("%s: %s [%d] associated to domain %s\n", __func__, clk->name, clk->rate, clk->clkdm_name);
		clk->clkdm = clkdm;
	} else {
		//printk("%s: could not associate %s to  domain %s\n", __func__, clk->name, clk->clkdm_name);
	}
}
...
//	printk("%s: %x(%s %lu)\n", __func__, (clk)?(unsigned int)(clk->set_rate):0, (clk)?clk->name:"null", rate);
...
//	printk("%s: %s <-- %d\n", __func__, mpurate_ck_name, mpurate);
...
//	printk("%s: %s %ld <-> %ld\n", __func__, mpurate_ck_name, mpurate_ck->rate, mpurate);
//	printk("%s: %lu.%02lu BogoMIPS (lpj=%lu)\n", __func__,
