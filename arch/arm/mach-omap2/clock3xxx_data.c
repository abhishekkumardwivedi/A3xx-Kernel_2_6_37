...
static struct clk dpll5_m2_ck = {
	...
	.set_rate	= &omap3_noncore_dpll_set_divider_rate,
...
omap3xxx_clk_init(void)
{
	...
	for (c = omap3xxx_clks; c < omap3xxx_clks + ARRAY_SIZE(omap3xxx_clks); c++)
		if (c->cpu & cpu_clkflg) {
			//printk("%s: add clock %s [%d]\n", __func__, c->lk.clk->name, c->lk.clk->rate);
			clkdev_add(&c->lk);
...
//	printk("%s: Crystal/Core/MPU %ld.%01ld/%ld/%ld MHz\n", __func__,
...
//		(core_ck.rate / 1000000), (arm_fck.rate / 1000000));
	...
	printk("%s: SDRC clock %s, %lu Hz\n", __func__, sdrc_ick_p->name, sdrc_ick_p->rate);
