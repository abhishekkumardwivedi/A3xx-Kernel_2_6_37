/*
 * tps65930.c - Based on sound/soc/codecs/twl4030.c
 *
 * ALSA SoC TWL4030/TPS65930 codec driver for Micronet A300 device.
 *
 * A300 uses TPS65930, which its functionality differs from the full TPS65950
 * Especially the audio codec is completely different and supports only a subset of
 * the outputs/inputs and control register commands.
 *
 * Original Author:      Steve Sakoman, <steve@sakoman.com> (twl4030.c)
 * Author:      		 Ruslan Sirota, <ruslans@micronet.co.il> (tps65930.c)
 *
...
static const u8 twl4030_reg[TWL4030_CACHEREGNUM] = {
	...
	0x1B, /* REG_ARXL1_APGA_CTL	(0x19)	*/
	0x90, /* REG_ARXR1_APGA_CTL	(0x1A)	*/
	0x90, /* REG_ARXL2_APGA_CTL	(0x1B)	*/
#if defined(CONFIG_MACH_A300)
	0x90, /* REG_ARXR2_APGA_CTL	(0x1C)	*/
#else
...
static  reg_debug_st twl4030_reg_debug[TWL4030_CACHEREGNUM] = {
...
struct twl4030_priv {
	...
	u8 ext_mic_enabled, ext_spkr_enabled;
...
static void dump_reg_vals(struct snd_soc_codec *codec){
	u8 reg;
	CE300_SND_SOC_TRACE("0xidx\tCache\tHW-Val\tName\n");

	for (reg = 0; reg < TWL4030_CACHEREGNUM; reg++){
        u8 regVal = 0;
        twl_i2c_read_u8(TWL4030_MODULE_AUDIO_VOICE, &regVal,
        		reg);
		CE300_SND_SOC_TRACE("0x%2X\t%2X\t%2X\t%s\n",
				reg,
				twl4030_read_reg_cache(codec, reg),
				regVal,
				twl4030_reg_debug[reg].reg_name);
	}
}
#endif
...
static void set_codec_output_gain(struct snd_soc_codec *codec, int external){
	...
	dump_reg_vals(codec);
#endif
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX %s() entered , external = 0%d, int=%d, ext=%d\n", __FUNCTION__,
				external,
				twl4030->int_speaker_gpio,
				twl4030->ext_speaker_gpio);

	if(!(external)){

		// Set Predr gain to +6dB
//		regVal = (PREDL_OUTLOW_EN | PREDL_GAIN(1) | PREDL_AL1_EN);
//		twl_i2c_write_u8(TWL4030_MODULE_AUDIO_VOICE, regVal, TWL4030_REG_PREDL_CTL);
//
//		// Set Gain - +12 dB
//		regVal = (unsigned char) (APGA_CTL_PDZ | APGA_CTL_DA_EN | APGA_CTL_GAIN_SET(0x0) );
//
//		twl_i2c_write_u8(TWL4030_MODULE_AUDIO_VOICE, regVal, TWL4030_REG_ARXL1_APGA_CTL);
//
//		regVal = (0 << 6) | 0x3F; // +12dB -12dB = +0dB
//		twl_i2c_write_u8(TWL4030_MODULE_AUDIO_VOICE, regVal, TWL4030_REG_ARXL1PGA);
...
//
//		// Set Gain - +6 dB
//		regVal = (unsigned char) (APGA_CTL_PDZ | APGA_CTL_DA_EN | APGA_CTL_GAIN_SET(0x3) );
//
//		twl_i2c_write_u8(TWL4030_MODULE_AUDIO_VOICE, regVal, TWL4030_REG_ARXL1_APGA_CTL);
//
//		regVal = (2 << 6) | 0x33; // +12dB -12dB = +0dB
//		twl_i2c_write_u8(TWL4030_MODULE_AUDIO_VOICE, regVal, TWL4030_REG_ARXL1PGA);
...
static void set_codec_output_routing(struct snd_soc_codec *codec, int external){
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX %s() entered ,  external = %d\n", __FUNCTION__,
						external);

	if (twl4030->int_speaker_gpio != 0 && twl4030->ext_speaker_gpio != 0) {
		switch (external) {
		case 0: //Internal Audio
		case 4: //Default state set when codec is enabled
			mdelay(25);
			if (twl4030->codec_powered) gpio_direction_output(twl4030->ext_speaker_gpio, 0);
			if (twl4030->codec_powered) gpio_direction_output(twl4030->int_speaker_gpio, 1);

			twl4030->ext_spkr_enabled = 0;
			break;
		case 1:
			mdelay(25);
			if (twl4030->codec_powered) gpio_direction_output(twl4030->int_speaker_gpio, 0);
			if (twl4030->codec_powered) gpio_direction_output(twl4030->ext_speaker_gpio, 1);

			twl4030->ext_spkr_enabled = 1;
			break;
		case 2: //Cradle audio switch option
			if (twl4030->cradle_aud_gpio != 0) {
...
static void set_codec_input_routing(struct snd_soc_codec *codec, int external){
	...
	dump_reg_vals(codec);
#endif
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX %s() entered ,  external = %d, ext mic gpio =%d\n", __FUNCTION__,
				external,
				twl4030->ext_mic_gpio);

	if(twl4030->ext_mic_gpio != 0){
		if(!(external)){
			...
			unsigned int reg, unsigned int value)
{
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX %s() entered , codec=%s, \n reg=%x (< 0x4A?), value=%x\n", __FUNCTION__, codec->name, reg, value);

//	if(reg != TWL4030_REG_VIBRA_CTL && reg != TWL4030_REG_VIBRA_SET && reg != TWL4030_REG_VIBRA_PWM_SET)
...
static void twl4030_codec_enable(struct snd_soc_codec *codec, int enable)
{
	struct twl4030_priv *twl4030 = snd_soc_codec_get_drvdata(codec);
	int mode = 0;
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX %s() entered \n", __FUNCTION__);
	if (enable == twl4030->codec_powered)
		return;

	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX %s() enable=%d\n", __FUNCTION__, enable);
	//printk("twl4030_codec_enable\n");
...
static void twl4030_apll_enable(struct snd_soc_codec *codec, int enable)
{
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX twl4030_apll_enable() entered \n");
	if (enable) {
...
/* A300 only - External/Internal speaker selection */
...
static const struct soc_enum twl4030_speakerroute_enum =
	SOC_ENUM_SINGLE(TWL4030_REG_DUMMY_FOR_SPEAKER, 0,
			ARRAY_SIZE(twl4030_speakerroute_texts),
			twl4030_speakerroute_texts);

static const struct snd_kcontrol_new twl4030_dapm_speakerroute_control =
SOC_DAPM_ENUM("Route", twl4030_speakerroute_enum);

static const char *twl4030_microute_texts[] =
		{"Internal", "External"};
...
{									\
	struct twl4030_priv *twl4030 = snd_soc_codec_get_drvdata(w->codec); \
									\
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX %s() entered , widget name =%s,\n sname=%s, reg=%x, mask=%d\n", __FUNCTION__, \
			w->name, w->sname, reg, mask); \
	switch (event) {						\
...
//static int predrivelpga_event(struct snd_soc_dapm_widget *w,
//		struct snd_kcontrol *kcontrol, int event)
//{
//	struct twl4030_priv *twl4030 = snd_soc_codec_get_drvdata(w->codec);
//
...
//		twl4030_write(w->codec, 0x25,
//			twl4030_read_reg_cache(w->codec, 0x25));
//		break;
//	case 0x8:
//		twl4030->predrivel_enabled = 0;
//		twl_i2c_write_u8(0x01,
//					0, 0x25);
//		break;
//	}
//	return 0;
...
static int apll_event(struct snd_soc_dapm_widget *w,
		struct snd_kcontrol *kcontrol, int event)
{
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX apll_event() entered \n");
	switch (event) {
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX aif_event() entered \n");
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX snd_soc_get_volsw_twl4030() entered \n");
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX snd_soc_put_volsw_twl4030() entered \n");
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX snd_soc_put_volsw_twl4030() max=%d, mask=%d, \nreg=%x, val_mask=%d, \nval=%d, shirt=%d, rshift=%d\n", max
			,mask, reg, val_mask, val, shift, rshift);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX snd_soc_put_volsw_twl4030() max=%d, mask=%d, \nreg=%d, val_mask=%d, \nval=%d, shirt=%d\n", max
			,mask, reg, val_mask, val, shift);
	err = snd_soc_update_bits(codec, reg, val_mask, val);
	if (err < 0)
		return err;

	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX snd_soc_put_volsw_twl4030() max=%d, mask=%d, \nreg=%d, val_mask=%d, \nval2=%d, shirt=%d\n", max
			,mask, reg, val_mask, val2, shift);
...
//		0, 0x3f, 0, digital_fine_tlv),
	SOC_SINGLE_TLV("Speaker Digital Fine Volume",
		TWL4030_REG_ARXL1PGA, 0, 0x3f, 0, digital_fine_tlv),
...
//		6, 0x2, 0, digital_coarse_tlv),
	SOC_SINGLE_TLV("Speaker Digital Coarse Volume",
		TWL4030_REG_ARXL1PGA, 6, 0x2, 0, digital_coarse_tlv),
...
//		3, 0x12, 1, analog_tlv),
	SOC_SINGLE_TLV("Speaker Analog Volume",
		TWL4030_REG_ARXL1_APGA_CTL, 3, 0x12, 1, analog_tlv),
//	SOC_DOUBLE_R_TLV("DAC2 Analog Playback Volume",
//		TWL4030_REG_ARXL2_APGA_CTL, TWL4030_REG_ARXR2_APGA_CTL,
//		3, 0x12, 1, analog_tlv),
	SOC_SINGLE("Speaker Analog Switch",
		TWL4030_REG_ARXL1_APGA_CTL, 1, 1, 0),
...
//		4, 3, 0, output_tvl),
	SOC_SINGLE_TLV_TWL4030("Speaker Master Volume",
		TWL4030_REG_PREDL_CTL, 4, 3, 0, output_tvl),

	/* Common capture gain controls */
	SOC_DOUBLE_R_TLV("Microphone Digital Volume",
...
//		0, 0x1f, 0, digital_capture_tlv),
	...
	SOC_DOUBLE_TLV("Microphone Analog Volume", TWL4030_REG_ANAMIC_GAIN,
...
//		SND_SOC_NOPM, 0, 0),
	...
	SND_SOC_DAPM_MUX("Microphone Capture Route", SND_SOC_NOPM, 0, 0,
...
//		&twl4030_dapm_micpathtx2_control),
	...
	SND_SOC_DAPM_MUX("Microphone Route", SND_SOC_NOPM, 0, 0,
		&twl4030_dapm_microute_control),
	SND_SOC_DAPM_MUX("Speaker Route", SND_SOC_NOPM, 0, 0,
		&twl4030_dapm_speakerroute_control),
			     ...
			     struct snd_soc_dai *dai)
{
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX twl4030_shutdown() entered \n");
			   ...
			   struct snd_soc_dai *dai)
{
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX twl4030_hw_params() entered \n");
	 /* If the substream has 4 channel, do the necessary setup */
	if (params_channels(params) == 4) {
		format = twl4030_read_reg_cache(codec, TWL4030_REG_AUDIO_IF);
		mode = twl4030_read_reg_cache(codec, TWL4030_REG_CODEC_MODE);

		CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX twl4030_hw_params() 1 \n");
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX twl4030_hw_params() params_rate(params)=%d \n",params_rate(params));
	...
	case SNDRV_PCM_FORMAT_S16_LE:
		...
		CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX twl4030_hw_params() params_rate(format)=%d \n",params_format(params));
		break;
	...
	if (format != old_format || mode != old_mode) {
		if (twl4030->codec_powered) {
			 ...
			 * codec power.
			 */
			CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX twl4030_hw_params() codec_powered = true \n");
			twl4030_codec_enable(codec, 0);
		...
		int clk_id, unsigned int freq, int dir)
{
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX twl4030_set_dai_sysclk() freq = %d \n", freq);
	switch (freq) {
	case 19200000:
	case 26000000:
	case 38400000:
			     ...
			     unsigned int fmt)
{
	...
	if (format != old_format) {
		if (twl4030->codec_powered) {
			CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX %s() codec_powered == 1, toggle codec power \n", __FUNCTION__);
			/*
			 * If the codec is powered, than we need to toggle the
			 * codec power.
			 */
...
static int twl4030_set_tristate(struct snd_soc_dai *dai, int tristate)
{
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX twl4030_set_tristate() entered \n");
	if (tristate)
...
static int twl4030_set_digital_mute(struct snd_soc_dai *dai, int mute)
{
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX twl4030_set_digital_mute() entered mute=%d, ext_spkr_enabled=%d",
			mute, twl4030->ext_spkr_enabled);
	...
	// Input
//	gpio_direction_output(twl4030->ext_mic_gpio, (!mute) && (twl4030->ext_mic_enabled));
				...
				int enable)
{
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX twl4030_voice_enable() entered direction=%d, enable=%d\n", direction, enable);
	if (direction == SNDRV_PCM_STREAM_PLAYBACK)
		...
		struct snd_soc_dai *dai)
{
	 ...
	 * not avilable.
	 */
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX twl4030_voice_startup() entered \n");
				...
				struct snd_soc_dai *dai)
{
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX twl4030_voice_shutdown() entered \n");
		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX twl4030_voice_hw_params() entered \n");
	switch (params_rate(params)) {
	case 8000:
		...
		int clk_id, unsigned int freq, int dir)
{
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX twl4030_voice_set_dai_sysclk() entered \n");
	if (freq != 26000000) {
		...
		unsigned int fmt)
{
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX twl4030_voice_set_dai_fmt() entered \n");
	/* set master/slave audio interface */
	switch (fmt & SND_SOC_DAIFMT_MASTER_MASK) {
	case SND_SOC_DAIFMT_CBM_CFM:
...
static int twl4030_soc_probe(struct snd_soc_codec *codec)
{
	...
	printk("twl4030_soc_probe() codec. Entered\n");
...
static int __devinit twl4030_codec_probe(struct platform_device *pdev)
{
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX twl4030_codec_probe() entered \n");
	if (!pdata) {
...
static int __devexit twl4030_codec_remove(struct platform_device *pdev)
{
	...
	CE300_SND_SOC_TRACE("XXXXXXXXXXXXXXXXXXXX twl4030_codec_remove() entered \n");
...
MODULE_DESCRIPTION("ASoC TWL4030/TPS65930 codec driver");
MODULE_AUTHOR("Steve Sakoman / Ruslan Sirota");
MODULE_LICENSE("GPL");
